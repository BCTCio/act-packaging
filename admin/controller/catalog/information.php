...
class ControllerCatalogInformation extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$id = $this->model_catalog_information->addInformation($this->request->post);
            ...
            // 发送邮件
            $data = array();
            $data['message'] = 'You are receiving the updates from act. Please click button below to view details. Thanks.';
            $data['link_url'] = HTTP_CATALOG.'index.php?route=information/information&information_id='.$id;;
            $html = $this->load->view('mail/subscribe.tpl', $data);
...
//                    $mail->setSender(html_entity_decode('Jiaherb', ENT_QUOTES, 'UTF-8'));
//                    $mail->setSubject(html_entity_decode('subscribe info', ENT_QUOTES, 'UTF-8'));
//                    $mail->setHtml($html);
//                    $mail->send();
//                }
//            }
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['category'])) {
            ...
            $data['category'] = $information_info['category_id'];
        } else {
            $data['category'] = 0;
        }
