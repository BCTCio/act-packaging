...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$pid = $this->model_catalog_product->addProduct($this->request->post);
            ...
            // 发送邮件
            $data = array();
            $data['message'] = 'You are receiving the updates from jiaherbinc.com. Please click button below to view details. Thanks.';
            $data['link_url'] = HTTP_CATALOG.'index.php?route=product/category&filter=A';
            ...
            foreach ($emails as $email) {
                if (preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $email)) {
                    ...
                    $mail->setSubject(html_entity_decode('you received updates from jiaherbinc.com', ENT_QUOTES, 'UTF-8'));
                    $mail->setHtml($html);
                    $mail->send();
                }
            }
